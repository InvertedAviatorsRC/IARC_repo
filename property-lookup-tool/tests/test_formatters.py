...
def test_formatter_explains_free_market_data_gaps_and_prints_sources():
    data = PropertyData(
        ...
        parcel_id="260020210",
        source="Scott County MN public parcel data",
        source_urls=["https://example.gov/arcgis/rest/services/parcels/0"],
    )

    output = format_property_summary(data)

    assert "Parcel ID: 260020210" in output
    assert "Estimated Value: Not available from free public source" in output
